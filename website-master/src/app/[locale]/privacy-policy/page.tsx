...
export async function generateMetadata({
    ...
    const pageData = await fetchDataSEO('privacy-policy', locale, 'seo')
    ...
    return {
        ...
        alternates: {
            canonical: SITE_URL + getLangSlug(locale) + '/privacy-policy',
            languages: {
                uk: SITE_URL + '/privacy-policy',
...
const Page = async ({ params }: { params: { locale: AllowedLangs } }) => {
    const { locale } = params
    const pageData = await fetchData('privacy-policy', locale)
